module Shoppe
  class OrderItem < ActiveRecord::Base
    ...
    before_save do
      if order.received? && (unit_price_changed? || unit_cost_price_changed? || unit_special_price_changed? || tax_rate_changed? || tax_amount_changed?)
        cache_pricing
      end
    end
    ...
    # @return [BigDecimal]
    def unit_special_price?
      self.unit_special_price.present? && self.unit_special_price > 0 && self.unit_special_price < self.unit_price
    end
