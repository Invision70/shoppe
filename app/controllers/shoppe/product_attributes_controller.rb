module Shoppe
  class ProductAttributesController < Shoppe::ApplicationController
    ...
    def create
      ...
      if product_attributes.blank?
        ...
        product_attributes.each do |attribute_value|
          Shoppe::ProductAttribute.create(attribute_value.merge({key: safe_params[:product_attributes][:key]}).permit(:key, :value, :searchable, :public))
        end
    ...
    def update
      ...
      Shoppe::ProductAttribute.where(key: @product_attribute.key).update_all(key: safe_params[:product_attributes][:key])
